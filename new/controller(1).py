...
# 
    ...
    def _switch_packet_in_handle(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP and pkt.opcode == arp.ARP_REQUEST:
            for (dpid, src_mac, dst_ip_arp) in self.arp_table.keys():
                if dst_ip == dst_ip_arp:
                    self.logger.info("过滤掉ARP请求数据包: 源 IP=%s, 目标 IP=%s, 源 MAC=%s, 目标 MAC=%s, 交换机=%s, 端口=%s",
...
#_lldp_packet_in_handle 和 _host_arp_packet_in_handle 方法：分别处理LLDP和ARP数据包，用于发现链路和主机信息。
    ...
    def _host_ip_packet_in_handle(self, ev):
        ...
        # self.logger.info("000000_host_ip_packet_in_handle收到 IP 数据包: 源 IP=%s, 目标 IP=%s, 源 MAC=%s, 目标 MAC=%s, 交换机=%s, 端口=%s", 
                    # src_ip, dst_ip, src_mac, dst_mac, dpid, in_port)
        if eth.ethertype == ether_types.ETH_TYPE_IP or (eth.ethertype == ether_types.ETH_TYPE_ARP and pkt_type.opcode == arp.ARP_REPLY):
            pkt, _, _ = pkt_type.parser(pkt_data)
            src_ip = pkt.src
            dst_ip = pkt.dst
