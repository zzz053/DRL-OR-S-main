...
# 添加server配置
SERVER_CONFIG = {
    'server_ip': '127.0.0.1',  # 修改为server_agent的实际IP
    'server_port': 5001,
...
class TopoAwareness(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #各种标志位的开关
        self.show_enable = False  # 控制show方法的开关，True为开启，False为关闭
        self.host_migration_log_enable = False  # 控制主机迁移相关日志的开关
...
# 
    ...
    def _get_access_delay(self, src, dst):
        ...
        try:
            ...
            # 检查echo_latency字典中是否有源交换机的echo延迟
            if src not in self.echo_latency:
                self.logger.debug("交换机 %s 没有echo延迟信息（接入链路）", src)
                return float('inf')
            ...
            if eth.ethertype == ether_types.ETH_TYPE_LLDP:
                ...
                try:
                    ...
                    if len(parsed_values) < 2:
                        self.logger.warning("LLDP解析返回值不足: 期望至少2个值，实际得到%d个, dpid=%s, port=%s", 
                                          len(parsed_values), dpid, port)
                        return
                    
                    # 提取基本值（至少2个）
                    src_dpid = parsed_values[0]
                    src_port_no = parsed_values[1]
                    
                    # 如果返回值有4个，说明包含时间戳和echo延迟
                    if len(parsed_values) >= 4:
                        timestamp = parsed_values[2]
                        echodelay = parsed_values[3]
                        now_time = time.time()
                        lldpdelay = now_time - timestamp
                        # 保存延迟信息（包含echo延迟）
                        self._save_lldp_delay(src=dpid, dst=src_dpid, lldpdelay=lldpdelay, echodelay=echodelay)
                        self.logger.debug("收到LLDP包: src=%s, dst=%s, lldpdelay=%.6f, echodelay=%.6f", 
                                        src_dpid, dpid, lldpdelay, echodelay)
                    else:
                        # 只有2个值的情况，无法计算延迟，使用默认值
                        now_time = time.time()
                        lldpdelay = 0
                        echodelay = 0
                        # 只保存基本信息
                        self._save_lldp_delay(src=dpid, dst=src_dpid, lldpdelay=lldpdelay, echodelay=echodelay)
                        self.logger.debug("收到标准LLDP包（无时间戳）: src=%s, dst=%s", src_dpid, dpid)
                    ...
                    if src_dpid not in self.dpid_to_switch.keys():  # 该交换机并不属于自己管理的自组域.
                        ...
                        if (dpid, src_dpid) not in self.topo_access_link:
                            ...
                            # self.logger.info("Received LLDP packet333333: dpid=%s, src_dpid=%s, port=%s", dpid, src_dpid, port)
                    ...
                    # 捕获所有其他异常，记录日志但继续运行
                    self.logger.exception("处理LLDP数据包时发生异常: dpid=%s, port=%s", dpid, port)
                    return
        except Exception as e:
            # 捕获数据包解析等外层异常
            self.logger.exception("_lldp_packet_in_handle 处理数据包时发生异常")
            return
...
#_lldp_packet_in_handle 和 _host_arp_packet_in_handle 方法：分别处理LLDP和ARP数据包，用于发现链路和主机信息。
    ...
    def _host_arp_packet_in_handle(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP:
            ...
            # if [host.mac, host.ipv4] not in self.host_to_sw_port[dpid][in_port]:
                # self.host_to_sw_port[dpid][in_port].append([host.mac, host.ipv4])

            # if [src_mac, src_ip] not in self.host_to_sw_port[dpid][in_port]:
                # self.host_to_sw_port[dpid][in_port].append([src_mac, src_ip])
