...
class TopoAwareness(app_manager.RyuApp):
        ...
        for stat in sorted(body, key=attrgetter('port_no')):
            ...
            if port_no != ofproto_v1_3.OFPP_LOCAL:
                ...
                if key[0] in self.port_loss_stats and key[1] in self.port_loss_stats[key[0]]:
                    ...
                    prev_stats = self.port_stats[key][-2]  # 获取上一次的统计数据
                    
                    # 计算这个周期内的变化量
                    rx_packets_delta = stat.rx_packets - prev_stats[1]  # 接收包数变化
                    tx_packets_delta = stat.tx_packets - prev_stats[0]  # 发送包数变化
                    rx_dropped_delta = stat.rx_dropped - prev_rx_dropped  # 接收丢包变化
                    tx_dropped_delta = stat.tx_dropped - prev_tx_dropped  # 发送丢包变化
    ...
    def link_timeout_detection(self, access_link):  #  access_link，这是一个字典，通常用于存储链路信息，其中键是源和目标节点的元组，值是与链路相关的属性（例如时间戳）
        ...
        while True:
            ...
            for (src, dst) in list(link_lists.keys()):
                if (now_timestamp - link_lists[(src, dst)][1]) > 5:  # 当前的时间戳与该链接的最后更新时间戳。
                    try:
    ...
    def _switch_packet_in_handle(self, ev):
        ...
        # 这里的mac_to_port 目的MAC地址（主机）与交换机出端口的映射
...
#_lldp_packet_in_handle 和 _host_arp_packet_in_handle 方法：分别处理LLDP和ARP数据包，用于发现链路和主机信息。
    ...
    def _host_arp_packet_in_handle(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP:
            ...
            # 关键：检查是否是交换机间的链路端口
            if self.is_link_port(dpid, in_port):  # 如果输入端口是交换机间的链接端口，停止处理
                return
